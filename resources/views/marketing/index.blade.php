...
@section('content')
  {{-- Hero --}}
  <section class="mx-auto flex w-full max-w-3xl flex-col space-y-4">
    <h1 class="text-center text-7xl font-bold tracking-tight">Marketing for your nifty <span class="text-primary-600">SaaS
        Application</span></h1>
    <h2 class="py-4 text-lg font-medium">{{ config('app.name') }} is powered by a starter kit for early stage SaaS
      startups.
      Built with Laravel, Tailwind CSS, Inertia and Vue. You can quickly create any SaaS with this
      starter.</h2>
  ...
  <section class="mt-24">
    ...
    <p class="text-background-600 mt-4 text-center text-lg">{{ config('app.name') }} comes batteries included. It takes
      the best
      ...
      @foreach ($features as $feature)
        <div class="flex items-start gap-4">
          ...
          <div>
            ...
            <p class="text-background-500 mt-2 leading-relaxed">{{ $feature['description'] }}</p>
          </div>
        </div>
      @endforeach
    </div>
  ...
  <section class="bg-primary-600 my-24 rounded-lg">
    <div class="mx-auto max-w-7xl p-20">
      <div class="space-y-10">
        <h2 class="text-3xl font-bold tracking-tight text-white">The world's most innovative companies use our
          SaaS</h2>
  ...
  <section class="mt-24">
    ...
    <p class="text-background-600 mt-4 text-center text-lg">Choose an affordable plan that's packed with the best
      features for
    ...
    <div class="mx-auto my-16 grid max-w-7xl grid-cols-3 gap-x-8 space-y-0">
      @foreach ($plans as $plan)
        <div
          class="border-background-200 dark:border-background-700 dark:bg-background-800 relative flex flex-col rounded-2xl border bg-white p-8 shadow-sm"
        >
          <div class="flex-1">
            ...
            @if ($plan['popular'])
              <p
                ...
                Most popular</p>
            @endif
            <p class="text-background-500 dark:text-background-400 mt-6">{{ $plan['description'] }}</p>
            ...
            <ul class="mt-6 space-y-6" role="list">
              @foreach ($plan['features'] as $feature)
                <li class="flex">
                  ...
                  <span class="text-background-500 dark:text-background-400 ml-3">{{ $feature }}</span>
                </li>
              @endforeach
            </ul>
          </div>
          ...
          <a href="{{ route('sign-up') }}" @class([
              ...
              'bg-primary-50 dark:bg-primary-900 text-primary-700 dark:text-primary-50 hover:bg-primary-100 dark:hover:bg-primary-700' => !$plan[
                  'popular'
              ],
          ])>
            Start your 7 day free trial</a>
  ...
  >
    <div class="lg:grid lg:grid-cols-3 lg:gap-8">
      <div class="space-y-4">
        ...
        <p class="text-background-500 text-lg">Can't find the answer you're looking for? Reach out to our <a
        ...
        <dl class="space-y-12">
          @foreach ($faqs as $faq)
            <div>
              ...
              <dd class="text-background-500 mt-2 text-base">{{ $faq['answer'] }}</dd>
            </div>
          @endforeach
        </dl>
      </div>
